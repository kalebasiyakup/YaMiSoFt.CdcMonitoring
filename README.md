...
- **Alarmlar:** Slot inaktifliği, WAL kritik durumu, subscription hatası, sürekli artan lag, ardışık health check hatası **ve Veri Tutarlılık Kontrolü'nde bulunan tutarsızlık/hata** gibi durumları izler; eşik aşıldığında **e-posta** ile bildirir (anti-flap: aynı sorun için tekrar tekrar göndermez, durum düzelince otomatik kapatır). `/alerts` ekranı önem/tür/durum filtreleri, manuel **Yenile** butonu ve sayfalama ile geçmişi listeler.
- **Veri Tutarlılık Kontrolü:** Onaylanmış CDC ilişkileri için (aralığı dakika/saat/gün olarak `/settings`'ten seçilebilir bir sıklıkta) kaynak-hedef satır sayısı/checksum karşılaştırması yapar, tutarsızlık bulunursa hem toplu, **HTML şablonlu** bir **e-posta** raporuyla bildirir (konu/gövde artık sabit "haftalık" ifadesi kullanmaz — gövdedeki "Kontrol sıklığı" metni ayarlanan gerçek aralığı dinamik gösterir) hem de her ilişki için `/alerts`'te bir **Veri Tutarsızlığı** alarmı açar (tutarsızlık düzelince otomatik kapanır). Checksum yalnızca **kaynağın publication'ında gerçekten yayınlanan kolonlara** göre hesaplanır — hedef servisin kendi eklediği fazladan bir kolon (ör. bir bookkeeping alanı) checksum'ı asla etkilemez. `/reconciliation` ekranı durum filtresi (Eşleşti/Tutarsız), manuel **Yenile** butonu ve sayfalama ile sonuç geçmişini listeler; eski kayıtlar ayarlanabilir bir saklama süresinden sonra otomatik silinir.
...
- **Şema Kataloğu:** `/schema-catalog` ekranı, kayıtlı her bağlantının tablo/görünüm, kolon, indeks ve kısıt (PK/FK/unique/check) bilgisini periyodik olarak **salt-okuma** ile tarayıp kendi metadata veritabanında saklar — bir veri sözlüğü. Tablolar şemaya göre gruplu listelenir; arama hem tablo hem **kolon adında** çalışır ("bu kolon hangi tablolarda var"), publication'a dahil tablolar rozetle işaretlenir, bir satır genişletilince kolon/indeks/kısıt sekmeleri açılır. Katalog "güncel durum" olarak tutulur: kaynakta artık bulunmayan nesneler silinmez, `DroppedAt` ile işaretlenir. İki tarama arasındaki farklar (tablo/kolon eklendi-silindi, tip veya NULL kabulü değişti, indeks/kısıt değişti) `/schema-catalog/changes` ekranındaki **şema değişiklik günlüğüne** yazılır; bir bağlantının ilk taramasında (tüm katalog "yeni" görüneceği için) kayıt üretilmez. Her bağlantı "Şimdi Tara" ile talep üzerine de yenilenebilir; periyodik tarama Ayarlar'dan tamamen **kapatılabilir** (kapalıyken mevcut katalog verisi ve ekranlar olduğu gibi kalır, yalnızca elle tarama çalışır). `/schema-catalog/report` ekranı ise katalogun tamamını **tek düz liste** halinde (her satır bir kolon) tablo ve kolon **açıklamalarıyla** birlikte verir; veritabanı/şema/arama filtreleri, "yalnızca publication'a dahil", "yalnızca açıklaması olmayan kolonlar" (dokümantasyon boşluğu raporu) ve "silinenleri de göster" seçenekleriyle daraltılabilir. Listede aynı tabloya ait satırlar ortak bir zemin rengi ve blok başındaki ayraç çizgisiyle gruplanır (satır bazlı zebra yerine **tablo bazlı** renklendirme), böylece bir tablonun nerede bitip diğerinin nerede başladığı görünür. Liste sayfalanır — sayfa boyutu (25/50/100/250/500, varsayılan 50) ekrandan seçilir ve "90 kayıttan 1-50 arası gösteriliyor" bilgisi sayfalama kontrollerinin yanında durur. **"Excel'e Aktar"** düğmesi ise ekrandaki filtrelerin aynısıyla ancak **sayfalama olmadan** (filtreye uyan tüm satırlarla, en fazla 50.000 satır) bir `.xlsx` dosyası indirir. `/schema-check` ekranından farkı: bu ekran kalıcı ve aranabilir bir katalog tutar, o ise iki taraf arasında anlık bir karşılaştırma yapar.
- **Ayarlar:** Tüm çalışma parametreleri (tarama sıklıkları, alarm eşikleri, SMTP bilgileri, health check/reconciliation/şema kataloğu kayıt saklama süreleri) veritabanında saklanır ve `/settings` ekranından — her alanın yanındaki bilgi ikonuyla ne işe yaradığı açıklanarak, e-posta ayarları için kaydetmeden test gönderme imkânıyla, **yeniden başlatma gerekmeden** — yönetilir. Tarih/saat formatı (Türkçe/English) tercihi artık Ayarlar'da değil, sol menünün en altında (tarayıcı bazlı bir çerezle, hesap girişi olmadığından o tarayıcıya özel) seçilir.
...
src/
  ...
  CdcMonitoring.Application/     İş servisleri, arayüzler (Connections, CdcDiscovery,
                                  Alerting, Reconciliation, SchemaCatalog, Settings,
                                  HealthChecks)
